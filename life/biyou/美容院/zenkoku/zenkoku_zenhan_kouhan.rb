...
(0..99).each do |i|
  p "=======================#{i}"
  url = "http://search.loco.yahoo.co.jp/search?areacd=0#{fuga}2#{hoge}&genrecd=0419001&property=&ei=UTF-8&xargs=#{i+1}&b=#{i}1"
  doc=Nokogiri::HTML(open(url).read)
  CSV.open("zenkoku_zenhan_kouhan.csv", "a") do |csv| # CSVファイルを生成

...
(0..99).each do |i|
  p "==========================================#{i}"
  url = "http://search.loco.yahoo.co.jp/search?areacd=0120#{hoge}&genrecd=0419001&property=&ei=UTF-8&xargs=#{i+1}&b=#{i}1"
  doc=Nokogiri::HTML(open(url).read)
  CSV.open("biyouin_nonsapporo_1st.csv", "a") do |csv| # CSVファイルを生成
      if doc.xpath('//*[@id="S1ak"]/p/em').inner_text.include?('条件に一致する検索結果はありません。')

...
(0..99).each do |i|
  p "=======================#{i}"
  url = "http://search.loco.yahoo.co.jp/search?areacd=0#{fuga}20#{hoge}&genrecd=020900#{hogya}&property=&ei=UTF-8&xargs=#{i+1}&b=#{i}1"
  doc=Nokogiri::HTML(open(url).read)
  CSV.open("zenkoku_fashion.csv", "a") do |csv| # CSVファイルを生成

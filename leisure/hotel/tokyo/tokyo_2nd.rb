...
(0..99).each do |i|
  p "=======================#{i}"
  url = "http://search.loco.yahoo.co.jp/search?areacd=131#{hoge}&genrecd=030400#{hogya}&property=&ei=UTF-8&xargs=#{i+1}&b=#{i}1"
  doc=Nokogiri::HTML(open(url).read)
  CSV.open("hotel_tokyo.csv", "a") do |csv| # CSVファイルを生成
